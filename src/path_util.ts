...
let _svgPathExp = /[achlmqstvz]|(-?\d*\.?\d*(?:e[\-+]?\d+)?)[0-9]/ig,
    ...
    _atan2 = Math.atan2,
    _largeNum = 1e8,
    _isString = (value: any) => typeof (value) === "string",
    _isNumber = (value: any) => typeof (value) === "number",
    _isUndefined = (value: any) => typeof (value) === "undefined",
    _temp = {},
    _temp2 = {},
    _roundingNum = 1e5,
    _wrapProgress = (progress: number) => (Math.round((progress + _largeNum) % 1 * _roundingNum) / _roundingNum) || ((progress < 0) ? 0 : 1), //if progress lands on 1, the % will make it 0 which is why we || 1, but not if it's negative because it makes more sense for motion to end at 0 in that case.
    _round = (value: number) => (Math.round(value * _roundingNum) / _roundingNum) || 0,
    _roundPrecise = (value: number) => (Math.round(value * 1e10) / 1e10) || 0,
    _splitSegment = (rawPath: any[], segIndex: number, i: number, t: number) => {
    ...
    _getSampleIndex = (samples: any[], length: number, progress: number) => {
    ...
    _reverseRawPath = (rawPath: any[], skipOuter: undefined) => {
    ...
    _copyMetaData = (source: TMetadata, copy: TMetadata) => {
        copy.totalLength = source.totalLength;
        if ('samples' in source) { //segment
            (copy as typeof source).samples = source.samples.slice(0);
            (copy as typeof source).lookup = source.lookup.slice(0);
            (copy as typeof source).minLength = source.minLength;
            (copy as typeof source).resolution = source.resolution;
        } else if (source.totalPoints) { //rawPath
            (copy as typeof source).totalPoints = source.totalPoints;
        }
        return copy;
    },
    //pushes a new segment into a rawPath, but if its starting values match the ending values of the last segment, it'll merge it into that same segment (to reduce the number of segments)
    _appendOrMerge = (rawPath: any[], segment:any[]) => {
...
export function getRawPath(value: string | SVGElement) {
    value = ((_isString(value) && _selectorExp.test(value as string)) ? (document.querySelector(value as string) as SVGElement) || value : value);
    let e = ((value as SVGAElement).getAttribute ? value : 0) as any,
        rawPath;
    if (e && (value = (value as SVGAElement).getAttribute("d")!) as string) {
        //implements caching
        if (!e._gsPath) {
            e._gsPath = {};
        }
...
export function pointToScreen(svgElement: SVGElement, point: SVGPoint) {
